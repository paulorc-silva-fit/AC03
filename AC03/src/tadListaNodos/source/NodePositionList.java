...
public class NodePositionList<E> implements PositionList<E> {
	...
	protected DNode<E> checkPosition(Position<E> p) throws InvalidPositionException {
		...
		for(DNode<E> inicio = header.getNext(); inicio != trailer; inicio = inicio.getNext()) { 
			if(p == inicio) {
				conta++;
			}
			if(inicio.getNext() == trailer && conta == 0) { 
				throw new InvalidPositionException("This position is not a valid position");				
			}
		}
	...
	// Inverte uma lista de nodos
	public void reverseList(PositionList<E> lista) {
		...
		DNode<E> inicio = header.getNext();
		
		while (inicio != trailer) {
			inicio.setElement(pilha.pop()); 
